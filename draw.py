...
from img2linedraft import *

import pyrealsense2 as rs
from tf.transformations import quaternion_matrix

from draw3d.msg import sphere
from draw3d.srv import sphere_cam
...
class ExampleFullArmMovement:
    def __init__(self):
        try:
            ...
            #setup mocap client
            sphere_cam_service_name = '/draw3d'
            rospy.wait_for_service(sphere_cam_service_name)
            self.sphere = rospy.ServiceProxy(sphere_cam_service_name, sphere_cam)
        except:
    ...
    def example_cartesian_waypoint_action(self, wp_cmds):
        self.last_action_notif_type = None
        ...
        for i in range(wp_cmds.shape[0]):
            trajectory.waypoints.append(self.FillCartesianWaypoint(wp_cmds[i,0], wp_cmds[i,1], wp_cmds[i,2], wp_cmds[i,3], wp_cmds[i,4], wp_cmds[i,5], 0))
    ...
    def main(self, ):
        ...
        if success:
            ...
            # success &= self.example_send_cartesian_pose(0.3, 0.2, 0.4, 45.0, 0.0, 150)
            cam_sphere = self.sphere().sphere
            print(cam_sphere)
            pc = [cam_sphere.x, cam_sphere.y, cam_sphere.z]
            radius = cam_sphere.r
            input_type = 'text'
            # input_type = 'img'
            text = "人"
            ...
            size = 1280
            min_contour_len = 29
            gray = get_input(input_type, text, size)
            ...
            wp_cmd_list = map_to_sphere(waypoints, pc, radius, plane=False)
            for i in range(len(wp_cmd_list)):
                rospy.loginfo("Moving to waypoint " + str(i+1) + "/" + str(len(wp_cmd_list)))
                print(wp_cmd_list[i])
                if wp_cmd_list[i].shape[0] >= 300:
                    wp_cmd_list_array = np.array_split(wp_cmd_list[i], int(wp_cmd_list[i].shape[0]/300))
                    for i in range(len(wp_cmd_list_array)):
                        success &= self.example_cartesian_waypoint_action(wp_cmd_list_array[i])
                else:
                    success &= self.example_cartesian_waypoint_action(wp_cmd_list[i])
...
if __name__ == "__main__":
    ...
    # waypoints = generate_waypoints(edges, contours, min_contour_len)
    # # use detected Center coordinates
    # wp_cmd_list = map_to_sphere(waypoints)
